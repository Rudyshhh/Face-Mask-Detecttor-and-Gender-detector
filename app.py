...
# Detect and predict mask or gender
def detect_and_predict(frame, faceNet, model, task="mask"):
    (h, w) = frame.shape[:2]
...
def select_task():
    ...
    ttk.Label(root, text="Choose Detection Mode:", font=("Helvetica", 14)).pack(pady=20)
    ttk.Button(root, text="Mask Detection", command=lambda: proceed("mask")).pack(pady=10)
    ttk.Button(root, text="Gender Detection", command=lambda: proceed("gender")).pack(pady=10)
...
def create_ui():
    ...
    ttk.Label(root, text=f"{current_task.capitalize()} Detection", font=("Helvetica", 16)).pack(pady=20)
    ttk.Button(root, text="Image", command=lambda: select_input_mode("image")).pack(pady=10)
    ttk.Button(root, text="Video", command=lambda: select_input_mode("video")).pack(pady=10)
    ttk.Button(root, text="Webcam", command=lambda: select_input_mode("webcam")).pack(pady=10)
